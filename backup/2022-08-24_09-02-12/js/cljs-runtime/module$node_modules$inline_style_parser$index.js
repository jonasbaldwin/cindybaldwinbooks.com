shadow$provide.module$node_modules$inline_style_parser$index=function(global,require,module,exports){function trim(str){return str?str.replace(TRIM_REGEX,""):""}var COMMENT_REGEX=/\/\*[^*]*\*+([^/*][^*]*\*+)*\//g,NEWLINE_REGEX=/\n/g,WHITESPACE_REGEX=/^\s*/,PROPERTY_REGEX=/^(\*?[-#/*\\\w]+(\[[0-9a-z_-]+\])?)\s*/,COLON_REGEX=/^:\s*/,VALUE_REGEX=/^((?:'(?:\\'|.)*?'|"(?:\\"|.)*?"|\([^)]*?\)|[^};])+)/,SEMICOLON_REGEX=/^[;\s]*/,TRIM_REGEX=/^\s+|\s+$/g;module.exports=function(style,options){function updatePosition(str){var lines=
str.match(NEWLINE_REGEX);lines&&(lineno+=lines.length);lines=str.lastIndexOf("\n");column=~lines?str.length-lines:column+str.length}function position(){var start={line:lineno,column:column};return function(node){node.position=new Position(start);match(WHITESPACE_REGEX);return node}}function Position(start){this.start=start;this.end={line:lineno,column:column};this.source=options.source}function error(msg){var err=Error(options.source+":"+lineno+":"+column+": "+msg);err.reason=msg;err.filename=options.source;
err.line=lineno;err.column=column;err.source=style;if(options.silent)errorsList.push(err);else throw err;}function match(re){if(re=re.exec(style)){var str=re[0];updatePosition(str);style=style.slice(str.length);return re}}function comments(rules){var c;for(rules=rules||[];c=comment();)!1!==c&&rules.push(c);return rules}function comment(){var pos=position();if("/"==style.charAt(0)&&"*"==style.charAt(1)){for(var i=2;""!=style.charAt(i)&&("*"!=style.charAt(i)||"/"!=style.charAt(i+1));)++i;i+=2;if(""===
